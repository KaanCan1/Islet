...
// MARK: - Accent colour from artwork

extension Color {
    /// Averages the image, then pushes saturation up and lifts dark tones, so the
    /// result still reads as an accent against the panel's black background.
    static func dominant(of image: NSImage) -> Color? {
        ...
        for x in 0 ..< side {
            for y in 0 ..< side {
                ...
                // Near-black pixels drown the average out; skip them.
                let brightness = color.brightnessComponent
                guard brightness > 0.12 else { continue }
                total.r += color.redComponent
                total.g += color.greenComponent
                total.b += color.blueComponent
                samples += 1
            }
        }
        ...
        let average = NSColor(
            red: total.r / samples, green: total.g / samples, blue: total.b / samples, alpha: 1
        ).usingColorSpace(.deviceRGB)
        guard let average else { return nil }

        return Color(nsColor: NSColor(
            hue: average.hueComponent,
            saturation: min(1, average.saturationComponent * 1.45 + 0.1),
            brightness: max(0.72, average.brightnessComponent),
            alpha: 1
        ))
    }
}
